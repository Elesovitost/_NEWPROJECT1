(async function initRegionCp() {
    try {
        ...
        // 2. Skrytí LSTV (odstraníme jej ze záhlaví osy)
        code = code.replace(/'LSTV:', \{ btn: 'lstv', id: 'cp_lstv' \} /, '');
        ...
        // Tím izolujeme foraminální kořeny od těl obratlů (vLabel), aby popisy obratlů zůstaly správné
        code = code
        ...
        // fRoot = kořen ve foraminu, root = kořen paracentrálně (o jeden níž s respektem k C8)
        ...
        code = code.replace(/const segments = \[\s*[\s\S]*?\];/m, cervicalSegmentsBlock);
        ...
        // 7. Masivní přepis segmentů obratlů v textu a ID (Provádíme odspodu nahoru)
        const segmentMap = [
        ...
        // 8. Skrytí těla C2 v renderu tabulky (bezpečný regex až do konce řádku)
        code = code.replace(/\[\s*'C2',\s*\{ btn: 'shape', id: 'c2_shape'.*/g, "// Řádek C2 vynechán");
